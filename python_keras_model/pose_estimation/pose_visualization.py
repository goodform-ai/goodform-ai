...
import cv2
from pose_estimation.pose_data_types import Person
import numpy as np
...
def visualize(
  ...
  for person in list_persons:
    ...
    for edge_pair, edge_color in KEYPOINT_EDGE_INDS_TO_COLOR.items():
      # This additional edge_pair[0] < len(keypoints) check will ensure that you only try to access keypoints that exist in the keypoints list.
      if (edge_pair[0] < len(keypoints) and edge_pair[1] < len(keypoints) and
          keypoints[edge_pair[0]].score > keypoint_threshold and
